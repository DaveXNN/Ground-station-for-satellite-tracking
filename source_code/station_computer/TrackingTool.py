...
from datetime import datetime, timedelta, timezone                  # for operations with date and time
...
class TrackingTool(Tk):
    def __init__(self, configuration_file):
        ...
        with open(self.configuration_file) as json_file:            # open configuration file
            ...
            self.last_tle_update = datetime.strptime(conf['last_tle_update'], '%Y-%m-%d %H:%M:%S.%f%z')  # TLE update
        ...
        self.rl07 = Label(self, text='MQTT status:', bg=self.bg_color, fg=self.text_color)
        self.rl07.place(relx=0.525, rely=0.35, anchor=W)
        self.rl10 = Label(self, textvariable=self.first_pass_info, bg=self.bg_color, fg=self.text_color)
        self.rl10.place(relx=0.750, rely=0.42, anchor=CENTER)
        ...
        self.re6 = Entry(textvariable=self.mqtt_status, justify=CENTER, bg=self.bg_color, fg=self.text_color)
        self.re6.place(relx=0.625, rely=0.35, relwidth=0.1, anchor=W)
        ...
        self.shutdown_button = Button(self, text='Shut down rotator',
                                      ...
                                      fg=self.text_color, activebackground=self.bg_color)
        self.shutdown_button.place(relx=0.775, rely=0.35, relwidth=0.1, anchor=W)
        self.close_button = Button(self, text='Close program', command=self.close_window,
                                   bg=self.bg_color, fg=self.text_color, activebackground=self.bg_color)
        self.close_button.place(relx=0.875, rely=0.35, relwidth=0.1, anchor=W)
        ...
        self.update_r_info()
        self.listbox_init()
        update_time = datetime.now(timezone.utc) - self.last_tle_update
    ...
    def create_stat(self):                                          # create station object
        return create_station(str(datetime.now(timezone.utc)),
    ...
    def show_pred_data(self, sat, aos_time, max_time, los_time, aos_az, max_az, los_az, max_el):
        ...
        self.aos_az.set(str(aos_az))
        self.max_time.set(max_time.strftime('%H:%M:%S'))
        self.max_az.set(str(max_az))
        self.max_el.set(str(max_el))
        self.los_time.set(los_time.strftime('%H:%M:%S'))
        self.los_az.set(str(los_az))
...
class TrackedSatellite:
    def __init__(self, name):
        ...
        self.aos_time = datetime.now()
        self.max_time = datetime.now()
        self.los_time = datetime.now()
        self.aos_az = 0
        self.max_az = 0
        self.los_az = 0
        self.max_el = 0
        ...
        self.create_data_thread = Thread(self.create_data(delay=0))
    ...
    def create_data(self, delay):
        ...
        for orb in station.visibility(self.tle.orbit(), start=Date.now() + timedelta(seconds=delay),
            ...
            if orb.event and orb.event.info.startswith('AOS'):
                ...
                time_1 = self.aos_time.strftime('%Y-%m-%dT%H:%M:%S.%f+00:00')
                self.aos_time = datetime.strptime(time_1, '%Y-%m-%dT%H:%M:%S.%f%z')
            ...
            if orb.event and orb.event.info.startswith('LOS'):
                ...
                time_1 = self.los_time.strftime('%Y-%m-%dT%H:%M:%S.%f+00:00')
                self.los_time = datetime.strptime(time_1, '%Y-%m-%dT%H:%M:%S.%f%z')
                ...
                if self.max_el > app.min_max and self.elevs[0] < 0.1:
                    ...
                    self.create_data_thread = Thread(self.create_data(delay=self.delay + 5000))
                    self.create_data_thread.start()
                break
    ...
    def track(self):
        if not app.tracking:
            if self.aos_time > datetime.now(timezone.utc):
                ...
                app.show_pred_data(self.name, self.aos_time, self.max_time, self.los_time, self.aos_az, self.max_az,
                                   self.los_az, self.max_el)
